...
DISCOUNT_MODEL.prototype = {
    ...
    get_discount:()=>{
        ...
        const GET_THIS_MONTH_QUERY ="SELECT promo.*,p.product_name FROM promotion as promo INNER JOIN product as p ON promo.product_id = p.id where p.status=1 and MONTH(promo.start_date)=MONTH(CURDATE()) AND MONTH(promo.end_date)>=MONTH(CURDATE())";
    ...
    add_discount :body=>{
        ...
        const status_change_1 ='UPDATE product SET status =? WHERE id = ?';
           return new Promise((resolve,reject)=>{
               if(month == this_month){
                pool.query(status_change_1,[discount_data.status[1],discount_data._id],(err,status)=>{
                    ...
                    pool.query(status_change_1,[discount_data.status[2],discount_data._id],(err,status)=>{
                        console.log(status);
                        pool.query(INSERT_DISCOUNT_QUERY,(err,result)=>{
                            //console.log(result);
                            if(err) reject(err);
